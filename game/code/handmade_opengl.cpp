...
OpenGLDrawBoundsRecursive(sort_sprite_bound *Bounds, u32 BoundIndex)
{
    ...
    if(!(Bound->Flags & Sprite_DebugBox))
    {
        ...
        for(sprite_edge *Edge = Bound->FirstEdgeWithMeAsFront;
            Edge;
...
{    
    ...
    for(u32 SortEntryIndex = 0;
        SortEntryIndex < Prep->SortedIndexCount;
    ...
    if(GlobalShowSortGroups)
    {
        ...
        u32 GroupIndex = 0;
        for(u32 BoundIndex = 0;
            BoundIndex < BoundCount;
            ++BoundIndex)
        {
            sort_sprite_bound *Bound = Bounds + BoundIndex;
...
OpenGLManageTextures(texture_op *First)
{
        ...
        Op = Op->Next)
    {
        if(Op->IsAllocate)
        {
            *Op->Allocate.ResultHandle = OpenGLAllocateTexture(Op->Allocate.Width, 
                                                               Op->Allocate.Height, 
                                                               Op->Allocate.Data);
        }
        else
